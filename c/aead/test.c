...
void test_xoodyak_aead(const char *path) {
   FILE * fp;
   char * key = NULL;
   char * nonce = NULL;
   char * pt = NULL;
   ...
   if (fp == NULL)
       exit(EXIT_FAILURE);
   while ((key_len = getline(&key, &read, fp)) != -1) {
       if((nonce_len = getline(&nonce, &read, fp)) != -1){
           if((pt_len = getline(&pt, &read, fp)) != -1){
               if((ad_len = getline(&ad, &read, fp)) != -1){
                   if((ct_len = getline(&ct, &read, fp)) != -1){
                       ...
                       struct keykack x;
                       init_xoodyak(&x, datahex(key), ((key_len - 1) / 2), tmp, 0, tmp, 0);
                       absorb(&x, datahex(nonce), ((nonce_len - 1) / 2));
                       absorb(&x, datahex(ad), ((ad_len - 1) / 2));
                       uint8_t *tmp_pt =  datahex(pt);

                       encrypt(&x, tmp_pt, ((pt_len - 1) / 2));
                       uint8_t tag[16];
                       squeeze(&x, tag, 16);
                       uint8_t *hex_ct = datahex(ct);
                       int i = 0;
                       for(i = 0; i < ((pt_len - 1)/2); i++) {
                           assert(tmp_pt[i] == hex_ct[i]);
                       }

                       for(i = 0; i < 16; i++) {
                           assert(tag[i] == hex_ct[i+((pt_len - 1)/2)]);
                       }
                   }
               }
           }
...
void test_xoodyak_hash(char *path) {
   ...
   while ((msg_len = getline(&msg, &read, fp)) != -1) {
       if((md_len = getline(&md, &read, fp)) != -1) {
           struct keykack x;
           init_xoodyak(&x, tmp, 0, tmp, 0, tmp, 0);
           msg[msg_len - 1] = '\0';
           md[md_len - 1] = '\0';
...
void run_test() {
    ...
    test_xoodyak_aead("/home/test/Desktop/test_xoodyak/vectors/aead.raw");
    printf("Test Xoodyak aead succes.\nTest Xoodyak hash...\n");
